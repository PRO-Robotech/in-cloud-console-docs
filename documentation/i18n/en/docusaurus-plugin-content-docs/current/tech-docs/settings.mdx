...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Value used for prefilling, such as a string, number, object, or array.
        String values support <code>{`{index}`}</code> placeholders
        that are substituted from the current URL segments (see&nbsp;below).
      </td>
    </tr>
  </tbody>
</table>

<h3>Value Templating via partsOfUrl</h3>

The <code>value</code> field supports placeholders <code>{`{0}`}</code>, <code>{`{1}`}</code>, <code>{`{2}`}</code>, …,
which the BFF replaces with the corresponding segments of the current URL
(<code>location.pathname.split('/')</code>).

For example, if the user is on the page
<code>/openapi-ui/clusters/prod/namespaces/my-ns</code>, the segments are:
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>{`{0}`}</code></td>
      <td>0</td>
      <td><code>""</code> (empty string before the first <code>/</code>)</td>
    </tr>
    <tr>
      <td><code>{`{1}`}</code></td>
      <td>1</td>
      <td><code>openapi-ui</code></td>
    </tr>
    <tr>
      <td><code>{`{2}`}</code></td>
      <td>2</td>
...
:::info
**Templating rules:**
- Substitution is performed **only** in <code>value</code>; the <code>path</code> field is never templated.
- Templating works **recursively** — placeholders are replaced in strings at any nesting level (inside objects and arrays).
- If a segment with the given index does not exist in the URL, an empty string is substituted.
:::

<h4>Example with partsOfUrl Placeholders</h4>
...
In this example, when the form is opened on a page like <code>/openapi-ui/clusters/prod/…</code>,
the <code>metadata.name</code> field will be prefilled with <code>svc-prod</code>,
and <code>spec.selector.app</code> with <code>app-prod</code>.
