...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Заглушка для отображения статуса (403, 404, 500, success, warning, info). Три режима: ручной (статус, заголовок и подзаголовок задаются в YAML), автоматический (компонент считывает HTTP-ошибку из urlsToFetch по reqIndex) и проверка пустого ответа (по умолчанию включена — если массив по пути <code>itemsPath</code> (по умолчанию <code>.items</code>) пуст, компонент показывает 404; отключается через <code>checkEmpty: false</code>).</td>
      <td>
        <figure style={{ textAlign: 'center', display: 'contents' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <YamlModal
            ...
            yamlContent={dedent`
              {
                id: number | string
                reqIndex?: number       // индекс запроса из urlsToFetch для автоопределения ошибки
                itemsPath?: string       // путь до массива для проверки пустоты (по умолчанию ".items")
