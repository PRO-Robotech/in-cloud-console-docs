...
<p>
  Breadcrumb in in‑Cloud is an additional navigation element. It shows the path from the root level to the current page
  and helps users understand the context they are working in. Its main goal is to provide quick access to previous
  levels without returning to the sidebar or repeating a search.
</p>

<p>
  The trail is built declaratively from a YAML description. Each item has a key, display name, and link, and those links
  can use placeholders such as <code>\{cluster}</code>, <code>\{namespace}</code>, <code>\{apiGroup}</code>,
  <code>\{apiVersion}</code>, and <code>\{plural}</code>. This makes it possible to configure navigation flexibly for a
  specific workflow.
</p>
...
<p>
  It works for both user and administrative scopes. The configuration is declarative (YAML) and does not require code
  changes.
</p>
...
<p>
  `TableUriMapping` is a route configuration mechanism for Kubernetes resource table views in in‑Cloud. When a route is
  defined, rows and cells become clickable and lead to the corresponding resource pages.
</p>
<p>
  It works for both user and administrative scopes. The configuration is declarative (YAML) and does not require code
  changes.
...
<p>
  The <code>Factory</code> resource is used in the in‑Cloud UI to build interfaces declaratively from YAML descriptions.
  It acts as a description of a page or part of a page and includes:
</p>
...
<p>
  Data fetched through <code>urlsToFetch</code> is available in all components through JSONPath syntax, including nested
  access:
</p>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Unique identifier used by the UI to find and apply the overridden form to the target resource.</td>
      <td></td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>
        Array of field paths used to sort the form. For example, <code>["spec", "serviceRef", "apiVersion"]</code>. This
        defines the display order of fields in the form.
      </td>
      <td>
        <YamlModal
          yamlContent={dedent`
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Basic field type: <code>string</code>, <code>integer</code>, <code>boolean</code>, <code>object</code>, or{' '}
        <code>array</code>.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>Custom in‑Cloud UI extensions for rendering dictionaries, ranges, and dependencies between fields.</td>
    </tr>
  </tbody>
</table>
