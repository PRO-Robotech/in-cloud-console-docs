...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Status placeholder for displaying errors or empty states (403, 404, 500, success, warning, info). Two modes: manual (status, title and subtitle are set in YAML) and auto-detect (component reads the HTTP error from urlsToFetch by reqIndex)</td>
      <td>
        <figure style={{ textAlign: 'center', display: 'contents' }}>
