...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Заглушка для отображения статуса (403, 404, 500, success, warning, info). Три режима: ручной (статус, заголовок и подзаголовок задаются в YAML), автоматический (компонент считывает HTTP-ошибку из urlsToFetch по reqIndex) и «пустой список = 404» (флаг <code>emptyAsNotFound</code> — если K8s-запрос с <code>fieldSelector</code> вернул пустой <code>items[]</code>, компонент показывает 404).</td>
      <td>
        <figure style={{ textAlign: 'center', display: 'contents' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <YamlModal
            yamlContent={dedent`
              ...
              # Автоматический режим + emptyAsNotFound
              # Оборачивает контент как children — при ошибке/не найдено
              # показывается только Result, при успехе — дочерние компоненты
              - type: antdResult
                data:
                  id: pod-fetch-error
                  reqIndex: 0
            ...
            yamlContent={dedent`
              {
                ...
                emptyAsNotFound?: boolean // если true и items[] пуст — показывает 404 (для K8s list с fieldSelector)
