...
<h3>Изоляция ошибок по запросам (reqIndex)</h3>

<p>
  Если в <code>urlsToFetch</code> указано несколько запросов, ошибка в одном из них <strong>не приводит</strong> к падению
  всех компонентов на странице. Каждый компонент может указать, от какого именно запроса он зависит, через параметр
  <code>reqIndex</code>:
</p>

<CodeBlock language="yaml" title="Пример: два запроса, компоненты привязаны к req0">
  {dedent`
    urlsToFetch:
      - cluster: "{2}"       # req0 — основной ресурс
        apiVersion: "v1"
        plural: "nodes"
        fieldSelector: "metadata.name={7}"
      - apiGroup: "metrics.k8s.io"  # req1 — метрики (может быть недоступен)
        apiVersion: "v1beta1"
        cluster: "{2}"
        plural: "pods"
    data:
      - type: parsedText
        data:
          reqIndex: "0"       # зависит только от req0
          text: "{reqsJsonPath[0]['.items.0.metadata.name']['-']}"
  `}
</CodeBlock>
...
<ul>
  <li>
    Если <code>reqIndex</code> указан — компонент проверяет ошибку <strong>только</strong> для этого запроса.
    Ошибки других запросов игнорируются, и компонент рендерит данные.
  </li>
  <li>
    Если <code>reqIndex</code> <strong>не указан</strong> — компонент ведёт себя как раньше:
    если хотя бы один запрос упал, компонент показывает ошибку (backward compatible).
  </li>
  <li>
    Значение <code>reqIndex</code> — строка (<code>"0"</code>, <code>"1"</code>, ...), соответствующая индексу в массиве <code>urlsToFetch</code>.
  </li>
</ul>
...
<table>
  ...
  `}
    ...
    <tr>
      ...
      <td>Индексы запросов из <code>urlsToFetch</code>, критичных для отображения страницы. Если хотя бы один из указанных запросов вернул ошибку или пустой ответ — вместо контента отображается <code>antdResult</code> с наихудшей ошибкой (500 {'>'} 403 {'>'} 404). Заменяет необходимость вручную оборачивать <code>data</code> в <code>antdResult</code>. Если не указан — автоматическая обёртка не применяется.</td>
    </tr>
    <tr>
      <td>
      ...
      <td>Путь до массива для проверки пустоты в ответах <code>effectiveReqIndexes</code>. По умолчанию <code>".items"</code>. Если задан как массив — каждая позиция соответствует позиции в <code>effectiveReqIndexes</code> (например, <code>[".items", ".data.results"]</code> для комбинированных запросов K8s + Prometheus).</td>
    </tr>
    <tr>
      <td>
        <code>data[]</code>
...
<table>
  ...
  <tbody>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <YamlModal
            ...
            yamlContent={dedent`
              {
                id: number | string
                reqIndex?: string // индекс запроса для изоляции ошибок
    ...
    <tr>
      ...
      <td>Значение из JSONPath сопоставляемое с valueMap</td>
      <td>
        {/* <figure style={{ textAlign: 'center', display: 'contents' }}>
          <FancyboxImage src={`${getBasePrefix()}img/in-cloud/components/parsedText.drawio.svg`} style={{ width: '100%', maxWidth: 'none' }} />
          <figcaption style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: '#666', textAlign: 'center' }}>
      ...
      <td>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <YamlModal
            buttonLabel="Пример структуры"
            yamlContent={dedent`
              { id: number | string, reqIndex?: string // индекс запроса для изоляции ошибок
    ...
    <tr>
      ...
      <td>Компонент отображения Volumes в табличном представлении с указанием mount</td>
      <td>
        {/* <figure style={{ textAlign: 'center', display: 'contents' }}>
          <FancyboxImage src={`${getBasePrefix()}img/in-cloud/components/NodeTerminal.drawio.svg`} style={{ width: '100%', maxWidth: 'none' }} />
          <figcaption style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: '#666', textAlign: 'center' }}>
    ...
    <tr>
      ...
      <td>Заглушка для отображения статуса (403, 404, 500, success, warning, info). Три режима: ручной (статус, заголовок и подзаголовок задаются в YAML), автоматический (компонент считывает HTTP-ошибку из urlsToFetch по reqIndex) и проверка пустого ответа (по умолчанию включена — если массив по пути <code>itemsPath</code> (по умолчанию <code>.items</code>) пуст, компонент показывает 404; отключается через <code>checkEmpty: false</code>). <code>reqIndex</code> может быть числом или массивом — при массиве проверяются все указанные запросы и отображается наихудшая ошибка (500 {'>'} 403 {'>'} 404). <br/><br/><strong>Рекомендуемый способ:</strong> вместо ручного добавления <code>antdResult</code> в <code>data</code> используйте проп <code>effectiveReqIndexes</code> в спеке фабрики — обёртка будет применена автоматически.</td>
      <td>
        <figure style={{ textAlign: 'center', display: 'contents' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <YamlModal
            yamlContent={dedent`
              # Ручной режим — статичная заглушка
              - type: antdResult
                data:
                  id: result-403
                  status: "403"
              ...
              # Ручной режим с VisibilityContainer (когда данных нет)
              - type: VisibilityContainer
                data:
                  id: vc-no-data
              ...
              # Автоматический режим — reqIndex указывает на запрос
              - type: antdResult
                data:
                  id: error-display
                  reqIndex: 0

              # Автоматический режим с children-обёрткой
              # При ошибке или пустом ответе — показывается Result,
              # при успехе — дочерние компоненты. checkEmpty включён по умолчанию.
              - type: antdResult
                data:
                  id: pod-fetch-error
                  reqIndex: 0
              ...
              # Кастомный путь до массива (для не-K8s API)
              - type: antdResult
                data:
              ...
              # Per-request пути (K8s + Prometheus в одном reqIndex)
              - type: antdResult
                data:
                  id: combined-error
                  reqIndex: [0, 1]
              ...
              # Отключение проверки пустого массива
              - type: antdResult
                data:
                  id: allow-empty-list
                  reqIndex: 0
              ...
              # ── Рекомендуемый способ: effectiveReqIndexes ──
              # Вместо ручного antdResult в data — укажите
              # effectiveReqIndexes в спеке фабрики.
              # Обёртка применяется автоматически.
              #
              # spec:
              ...
              #         id: page-tabs
              #
              # С per-request путями (K8s + Prometheus):
              # spec:
              #   effectiveReqIndexes: [0, 1]
              #   effectiveItemsPath: [".items", ".data.results"]
            `}
          />
          ...
          <YamlModal
            ...
            yamlContent={dedent`
              {
                id: number | string
                reqIndex?: number | number[]  // индекс (или массив индексов) запроса из urlsToFetch для автоопределения ошибки
                itemsPath?: string | string[]  // путь (или массив путей по позиции reqIndex) для проверки пустоты (по умолчанию ".items")
                checkEmpty?: boolean     // проверять пустой массив → 404 (по умолчанию true, отключить: false)
                status?: 'success' | 'error' | 'info' | 'warning' | '403' | '404' | '500'
                title?: string          // поддерживает шаблоны (reqsJsonPath, partsOfUrl)
                subTitle?: string       // поддерживает шаблоны
                style?: CSSProperties
              }
            `}
          />
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
            ...
            <a href="https://ant.design/components/result#result-demo-403" target="_blank">Пример</a>
          </div>
        </div>
      </td>
    </tr>
...
<h3>Пример ресурса (формат через объект)</h3>
...
:::info
Оба формата поддерживаются. На стороне BFF объектный формат нормализуется в канонический

<code>spec.values[]</code> (массив <code>{`{ path, value }`}</code>), который используется всеми потребителями.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Подстановки для полей формы:
        <ul>
          <li>
            канонический формат — массив <code>spec.values[]</code> c <code>path/value</code>;
          </li>
    ...
    <tr>
      ...
      <td>
        Значение для подстановки (строка, число, объект или массив).
        Строковые значения поддерживают плейсхолдеры <code>{`{index}`}</code>,
        которые подставляются из сегментов текущего URL (см.&nbsp;ниже).
      </td>
    </tr>
  </tbody>
</table>

<h3>Шаблонизация значений через partsOfUrl</h3>

В поле <code>value</code> можно использовать плейсхолдеры <code>{`{0}`}</code>, <code>{`{1}`}</code>, <code>{`{2}`}</code>, …,
которые при обработке на стороне BFF заменяются на соответствующие сегменты текущего URL
(<code>location.pathname.split('/')</code>).

Например, если пользователь находится на странице
<code>/openapi-ui/clusters/prod/namespaces/my-ns</code>, то сегменты будут:
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>{`{0}`}</code></td>
      <td>0</td>
      <td><code>""</code> (пустая строка перед первым <code>/</code>)</td>
    </tr>
    <tr>
      <td><code>{`{1}`}</code></td>
      <td>1</td>
...
- Подстановка выполняется **только** в <code>value</code>, поле <code>path</code> не&nbsp;шаблонизируется.
- Шаблонизация работает **рекурсивно** — плейсхолдеры заменяются в&nbsp;строках на&nbsp;любом уровне вложенности (внутри объектов и&nbsp;массивов).
- Если сегмента с&nbsp;указанным индексом нет в&nbsp;URL, подставляется пустая строка.
:::

<h4>Пример с partsOfUrl-плейсхолдерами</h4>
...
В этом примере при открытии формы на&nbsp;странице <code>/openapi-ui/clusters/prod/…</code>
поле <code>metadata.name</code> будет предзаполнено значением <code>svc-prod</code>,
а&nbsp;<code>spec.selector.app</code>&nbsp;— значением <code>app-prod</code>.
