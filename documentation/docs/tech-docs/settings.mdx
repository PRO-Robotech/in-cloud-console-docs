...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Status placeholder for displaying errors or empty states (403, 404, 500, success, warning, info). Three modes: manual (status, title and subtitle are set in YAML), auto-detect (component reads the HTTP error from urlsToFetch by reqIndex), and emptyAsNotFound (treats an empty K8s list response as 404 — useful for detail pages that fetch a single resource via fieldSelector)</td>
      <td>
        <figure style={{ textAlign: 'center', display: 'contents' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <YamlModal
            yamlContent={dedent`
              ...
              # Auto-detect + emptyAsNotFound — for detail pages
              # (K8s list with fieldSelector returns 200 + empty items
              #  instead of 404 when a resource is missing)
              - type: antdResult
                data:
                  id: pod-fetch-error
                  reqIndex: 0
                  emptyAsNotFound: true
                  title: "Pod {8} is not found"
                  subTitle: "Namespace: {3}"
                children:
                  - type: antdFlex
                    # ... page content renders as children
            `}
          />
            ...
            yamlContent={dedent`
              {
                ...
                emptyAsNotFound?: boolean // treat empty K8s list (items: []) as 404
