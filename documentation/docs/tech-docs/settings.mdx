...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Заглушка для отображения статуса (403, 404, 500, success, warning, info). Два режима: ручной (статус, заголовок и подзаголовок задаются в YAML) и автоматический (компонент считывает HTTP-ошибку из urlsToFetch по reqIndex)</td>
      <td>
        <figure style={{ textAlign: 'center', display: 'contents' }}>
